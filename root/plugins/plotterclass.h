...
void Plotter::setDataSet(std::vector<TString> dataset_, std::vector<double> scales_, std::vector<TString> legends_, std::vector<int> colors_, 
TString DYEntry_)
{
  dataset.clear();
  scales.clear();
...
void Plotter::setDataSet(TString mode)
{
  ...
  sample.push_back("Wll");

  dataset.push_back("ee_qcdem2030.root");
  scales.push_back(Lumi*2.36E8 * 0.0106 /35729669);
...
// fill(dataset[0], -999, 1);
  ...
  while (i<hists.size()){
     ...
     TH1F *HIST=(TH1F*)drawhists[i]->Clone();
     size_t j;
     for (j=i+1; j<hists.size(); j++){
       std::cout<<"j= "<<j<<std::endl;
       if( !strcmp(legends[i], legends[j]) ){ HIST->Add(drawhists[j]);}
       else{break;};
       };

       TString filename=TString(drawhists[i]->GetName());
       filename.Append(sample[i]);
       if (dataset[i].Contains("ee_")) filename.Append("EE");
       else if (dataset[i].Contains("mumu_")) filename.Append("MuMu");
       else if (dataset[i].Contains("emu_")) filename.Append("EMu");
       else std::cout<<"Wrong decay channel:decay channel NOT selected"<<std::endl;
       HIST->Write(filename);
       i=j;  
   };
