#define IsoStudies_cxx


#include "IsoStudies.h"
#include <TH2.h>
#include <TStyle.h>
...
void IsoStudies::Begin(TTree * /*tree*/)
{
  ...
  const Double_t binseta[nbinseta+1] = {-2.4, -1.2, 1.2, 2.4};
  ...
  data = new TH2D("data2d","data2d", nbinseta, binseta, nbinspt, binspt);
  MC = new TH2D("MC2d","MC2d", nbinseta, binseta, nbinspt, binspt);
  datapassed = new TH2D("passeddata2d","passeddata2d", nbinseta, binseta, nbinspt, binspt);
  MCpassed = new TH2D("passedMC2d","passedMC2d", nbinseta, binseta, nbinspt, binspt);
...
void IsoStudies::SlaveBegin(TTree * /*tree*/)
{
...
Bool_t IsoStudies::Process(Long64_t entry)
{
  ...
  for(unsigned int i=0; i<lepPt->size(); i++){ //isocut and chargecut -> builds leptonvectors start with the second leading pt lepton!!!
    ...
    if(tempiso < isocut && *(lepType->begin()+i) == 1 && *(lepQ->begin()+i) != CheckCharge){ //
  ...
  if(b_step1 && b_step2 && b_step3 && !b_step4){     // select Z peak
    ...
    if(isData){
      IsoStudiesData0->Fill(VLepPt[lepno]);      // Fill histo pt dependent with first lepton
      data->Fill(VLepEta[lepno], VLepPt[lepno], 1);
    }
    if(DataType== dataset+"_dy"+dataset+"50inf.root"){
      IsoStudiesZMC0->Fill(VLepPt[lepno], PUweight); 
      MC->Fill(VLepEta[lepno], VLepPt[lepno], PUweight);
    }
    ...
    if(VLepPfIso[lepno]<isocut){ //new
      if(isData){
	IsoStudiesData1->Fill(VLepPt[lepno]);
	datapassed->Fill(VLepEta[lepno], VLepPt[lepno], 1);
      }
      if(DataType== dataset+"_dy"+dataset+"50inf.root"){
	IsoStudiesZMC1->Fill(VLepPt[lepno]);  
	MCpassed->Fill(VLepEta[lepno], VLepPt[lepno], PUweight);
      }
...
void IsoStudies::SlaveTerminate()
{
...
void IsoStudies::Terminate()
{
  ...
  f = new TFile(dataset+"_IsoStudies_"+lep+add+".root","RECREATE");

  //////////2d Stuff///////

  TH2D *data2d = (TH2D*) data->Clone();
  data2d->GetXaxis()->SetTitle("#eta");
  data2d->SetTitle("data2d");
  data2d->SetName("data2d");

  for(int binx = 1; binx < data->GetNbinsX()+1; binx++){
    for(int biny = 1; biny <  data->GetNbinsY()+1; biny++){
      double content=datapassed->GetCellContent(binx,biny) / data->GetCellContent(binx,biny);
      double err=sqrt((1-(datapassed->GetCellContent(binx,biny) / data->GetCellContent(binx,biny)))/data->GetCellContent(binx,biny));
  ...
  TH2D *MC2d = (TH2D*) MC->Clone();
  MC2d->GetXaxis()->SetTitle("#eta");
  MC2d->SetTitle("MC2d");
  MC2d->SetName("MC2d");

  for(int binx = 1; binx < MC->GetNbinsX()+1; binx++){
    for(int biny = 1; biny <  MC->GetNbinsY()+1; biny++){
      double content=MCpassed->GetCellContent(binx,biny) / MC->GetCellContent(binx,biny);
      double err=sqrt((1-(MCpassed->GetCellContent(binx,biny) / MC->GetCellContent(binx,biny)))/MC->GetCellContent(binx,biny));
  ...
  TH2D *corrfactors = (TH2D*) MC->Clone();
