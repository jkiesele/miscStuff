#define Analysis_EE_cxx

#include "Analysis_EE.h"
#include <TH2.h>
#include <TStyle.h>
#include "TLorentzVector.h"


void Analysis_EE::Begin(TTree * /*tree*/)
{
   TString option = GetOption();

}

void Analysis_EE::SlaveBegin(TTree * /*tree*/)
{
  totalEventCount=0;
   TString option = GetOption();
...
Bool_t Analysis_EE::Process(Long64_t entry)
{
  GetAllBranches(entry);

  totalEventCount++;
  if((int)(100*totalEventCount)%(int)(1.25e+07) ==0) std::cout << totalEventCount << "\t\tEvents processed \tca. " << (int) totalEventCount/1.25e+07* 100 << "%" << std::endl;
  ...
  float tempiso=100;
  unsigned int k=0;
//   if(eventNumber == 17215579 || eventNumber ==16476741 || eventNumber ==75080142 || eventNumber ==214284560 || eventNumber ==472843680 || eventNumber ==149299010 || eventNumber ==206630175 || eventNumber ==119969449 ){
// std::cout << eventNumber << '\t' << *(lepQ->begin()) << "   " << *(lepQ->begin()+1) << "\t   " << *(lepPfIso->begin()) << "   " << *(lepPfIso->begin()+1) << std::endl;
//   }
  ...
  for(unsigned int i=0; i<lepPt->size(); i++){ //isocut and chargecut -> builds leptonvectors
    ...
    if(tempiso < isocutmuons && *(lepType->begin()+i) == 1 && *(lepQ->begin()+i) != CheckCharge){ //fill muons TEST eta
    ...
    else if(tempiso < isocutelecs && *(lepType->begin()+i) == -1 && *(lepQ->begin()+i) != CheckCharge){ //fill elecs TEST eta
  ...
  //if(VLepPt.size()>0)  c_step0.fill(DataType, PUweight);
c_step0.fill(DataType, PUweight);



//   int firstCharge=100;
//   if(VLepPt.size() > 0) firstCharge=VLepQ[0];
//   k=1;
//   while(k<VLepPt.size()){            //////select leptons having different charge compared to first lep
//     if(firstCharge == VLepQ[k]){  
//       VLepPt.erase(VLepPt.begin()+k);
//       VLepEta.erase(VLepEta.begin()+k);
//       VLepCombIso.erase(VLepCombIso.begin()+k);
//       VLepPfIso.erase(VLepPfIso.begin()+k);
//       VLepE.erase(VLepE.begin()+k);
//       VLepPx.erase(VLepPx.begin()+k);
//       VLepPy.erase(VLepPy.begin()+k);
//       VLepPz.erase(VLepPz.begin()+k);
//       VLepQ.erase(VLepQ.begin()+k);
//       VLepType.erase(VLepType.begin()+k);
//     }
//     else k++;
//   }
  ...
  // Test if everything is fine with lepton order
  ...
  if(VLepPt.size() > 1){  //two isolated leptons create dimass out of highest pt pair
    ...
    if((dimass < 76 || dimass > 106))     b_step4=true; //dimass > 50 && 
...
void Analysis_EE::SlaveTerminate()
{


}

void Analysis_EE::Terminate()
{
  TFile *f = new TFile("dielec_Analysis.root","RECREATE");
